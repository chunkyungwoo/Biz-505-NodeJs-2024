...
정수값 : TINYINT, SAMLINT, INT(INTEGER), BIGINT
     TINYINT : 0 ~ 225(또는 -128 ~ 127)
     SMALINT : 0 ~ 65535(-32768 ~ 32767)
     INT : 0 ~ 2의 4승 -1(-2의 3승 ~ 2의 3승 -1)
     BIG INT : 0 ~ 2의 8승 -1(-2의 7승 ~ 2의 7승 -1)
실수값 : FLOAT , DUBLE
     FLOAT(M,D) : M = 정수자릿수, D = 소수이하 자릿수
     FLOAT(3,2) = 정수3자리 소수2자리
...
-- 기존의 Table 을 DROP 하고 다시 그 구조대로 CREATE TABLE 실행한다
TRUNCATE tbl_books;
SELECT * FROM tbl_books;
...
UPDATE tbl_books
SET publisher = '자바출판사', author = '노도강'
WHERE isbn = '0001';
...
INSERT, UPDATE, DELETE 가 반영되지 않은 데이터를 보게될수도 있다
...
-- ckw2434 라는 사용자를 localhost 에서만 접근할수 있도록 생성한다
CREATE USER 'ckw2434'@'localhost' IDENTIFIED BY '!Biz8080';
-- 하지만 새로 생성된 사용자는 아무런 권한이 없기때문에 할수 있는 일이 없다
...
-- ckw2434 사용자에게 bookDB2 데이터베이스에 대하여 권한을 부여하기
-- TABLE 생성, CRUD 를 수행할 수 있다
GRANT ALL PRIVILEGES ON bookDB2.*TO 'ckw2434'@'localhost';
-- 기존에 부여된 권한을 회수하기
REVOKE ALL PRIVILEGES ON *.* FROM 'ckw2434'@'localhost';
